...
areas/cosmos/src/AzureMcp.Cosmos/
  AzureMcp.Cosmos.csproj
  GeneratedSdk/
    spec.lock.json
    roots.json
...
```text
areas/cosmos/src/AzureMcp.Cosmos/GeneratedSdk/
  src/
    Generated/
    Shared/
```

`AzureMcp.Cosmos.csproj` compiles these files directly into `AzureMcp.Cosmos.dll`. The generated types retain their `Azure.ResourceManager.CosmosDB` namespaces; their assembly identity is not a compatibility requirement because they are an internal implementation dependency of the Cosmos area.
...
1. Include the projected generated and shared source directly in `AzureMcp.Cosmos.csproj` and remove its normal service-package reference.
